...
class Controller(object):
    ...
    def __get_changes(self, update = False): # -1 for need download, 1 for need upload, 0 for can be uploaded and downloaded
        ...
        for nbName, lNotes in fileDict.items():##遍历本地文件中的笔记本
            eNotes = noteDict.get(nbName)##查找本地笔记本在网上是否存在
            if eNotes is None: # notebook exists locally， not online
                r.append(([nbName], 0))
                continue
            delIndex = []
            for lNote in lNotes:#遍历本地文件中某个笔记本中的所有笔记
                for i, eNote in enumerate(eNotes):##遍历网络文件中某个笔记本（和本地笔记本系统）中的所有笔记
                    ...
                    if self.ls.lastUpdate < lNote[1]: # need upload
                        if self.ls.lastUpdate < eNote[1] and lNote[1] < eNote[1]: # need download
                            r.append(([nbName, lNote[0]], 0))# 0 for can be uploaded and downloaded
                        else:
                            r.append(([nbName, lNote[0]], 1))# 1 for need upload,
                    else:
                        if self.ls.lastUpdate < eNote[1] and lNote[1] < eNote[1]:
                            r.append(([nbName, lNote[0]], -1))# -1 for need download
                        # else:
                            # debug
        ...
        def _download_note(noteFullPath):
            ...
            if contentDict.get(noteFullPath[1]+'.md') is None:
                if contentDict.get(noteFullPath[1]+'.html') is None:
                    ...
                    fileNum = 1
                    while 1:
                        if contentDict.get(noteFullPath[1]+'(%s).html'%fileNum) is None: ##什么情况？
